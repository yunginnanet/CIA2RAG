...
while :; do
	...
	cat mullvad_trigger >/dev/null && _next=$(head -n 1 mullvad_relays) &&
		echo "switching to: $_next" && mullvad relay set location ${_next} >/dev/null &&
		mullvad reconnect && sleep 0.5
	while
